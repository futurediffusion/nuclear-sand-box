...
func get_placeables_in_chunk(cx: int, cy: int, item_ids: Array[String] = []) -> Array[Dictionary]:
	...
	var result: Array[Dictionary] = []
	if _placeables_projection != null:
		result = _placeables_projection.get_placeables_in_chunk(cx, cy, item_ids)
	_chunk_query_time_usec_total += Time.get_ticks_usec() - t0
	_chunk_query_calls += 1
	return result
...
func get_all_placeables_by_item_id(item_id: String) -> Array[Dictionary]:
	if _placeables_projection == null:
		return []
	return _placeables_projection.get_all_placeables_by_item_id(item_id)
...
func get_debug_snapshot() -> Dictionary:
	...
	return {
		...
		"persistent_cache_item_ids": projection_snapshot.get("persistent_cache_item_ids", 0),
		"persistent_cache_uid_count": projection_snapshot.get("persistent_cache_uid_count", 0),
		"persistent_incremental_apply_calls": projection_snapshot.get("persistent_incremental_apply_calls", 0),
		"persistent_incremental_apply_avg_usec": projection_snapshot.get("persistent_incremental_apply_avg_usec", 0.0),
		"persistent_full_rebuild_calls": projection_snapshot.get("persistent_full_rebuild_calls", 0),
		"persistent_full_rebuild_avg_usec": projection_snapshot.get("persistent_full_rebuild_avg_usec", 0.0),
