...
# ---------------------------------------------------------------------------
# Frases de reacción territorial — cuando el jugador construye/mina en zona bandida
# ---------------------------------------------------------------------------
...
## Distancia máxima (px²) para que un NPC reaccione a intrusión territorial.
const TERRITORY_REACT_RANGE_SQ: float = 1000.0 * 1000.0
## Cooldown mínimo (s) entre reacciones territoriales por grupo (evita spam).
const TERRITORY_REACT_COOLDOWN: float = 20.0

# territory_reaction_cooldown: group_id → RunClock timestamp de última reacción
...
## Llamado desde world.gd._check_territory_intrusion() cuando el jugador coloca
## o mina dentro del radio de influencia de un grupo bandido.
## Selecciona el NPC más cercano a la intrusión y le pone una burbuja reactiva.
## Cooldown por grupo: TERRITORY_REACT_COOLDOWN segundos entre reacciones.
func notify_territory_reaction(faction_id: String, group_id: String,
		intrusion_pos: Vector2, kind: String) -> void:
	if _bubble_manager == null or _npc_simulator == null:
		return
	# Cooldown por grupo — una burbuja cada 20s máximo
	var now: float = RunClock.now()
	if now - float(_territory_react_cooldown.get(group_id, 0.0)) < TERRITORY_REACT_COOLDOWN:
		return
	# Buscar el NPC del grupo más cercano a la intrusión, dentro del rango de reacción
	...
	# Elegir pool de frases según tipo de actividad
	var is_mining: bool = kind in ["copper_mined", "stone_mined", "wood_chopped"]
	var pool: Array = TERRITORY_MINE_PHRASES if is_mining else TERRITORY_BUILD_PHRASES
	var phrase: String = pool[randi() % pool.size()]
	_bubble_manager.show_actor_bubble(best_node, phrase, 4.0)
	Debug.log("territory", "[TERRITORY] reaction npc=%s group=%s kind=%s" % [
		best_id, group_id, kind])
