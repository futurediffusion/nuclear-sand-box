class_name FlowerPainter extends RefCounted

## Qué atlas_coords del GroundTileMap se consideran grass.
## Configurar desde world.gd según el tileset real.
var grass_atlas_coords: Array[Vector2i] = []

## Cuántas variantes hay en flowers.png (columnas × filas del sheet 16×16).
...
## Densidad de flores: fracción de tiles grass que tendrán flor (0.0–1.0).
...
func load_chunk(chunk_coords: Vector2i) -> void:
	var data: Array = WorldSave.get_flower_data(chunk_coords)
	if data.is_empty():
		data = _generate_flowers(chunk_coords)
		WorldSave.set_flower_data(chunk_coords, data)
...
func _is_grass(src_id: int, atlas: Vector2i) -> bool:
	# Mantenido por compatibilidad — la lógica principal usa TileData.terrain en _generate_flowers
...
func _build_multimesh(chunk_coords: Vector2i, flowers: Array) -> void:
	...
	# Agrupar por variante — un MultiMeshInstance2D por variante para UVs correctas
	...
	# Sheet organizado en grilla de 16×16 px
	...
	for raw_variant in by_variant.keys():
		...
		# UVs normalizados al rect de esta variante en el sheet
		...
		# ArrayMesh: quad centrado en origen, tamaño controlado por _flower_size
		var h: float = _flower_size * 0.5
		var arrays: Array = []
