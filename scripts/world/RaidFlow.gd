...
func _create_job(gid: String, intent: Dictionary) -> void:
	...
	Debug.log("raid", "[RF] job created — group=%s base=%s" % [gid, str(intent.get("base_center"))])
...
func _tick_attacking(job: Dictionary, gid: String) -> bool:
	...
	if attack_elapsed >= ATTACK_DURATION or total_elapsed >= MAX_RAID_DURATION:
		Debug.log("raid", "[RF] attack phase done — group=%s attack_t=%.0f total_t=%.0f" % [
			gid, attack_elapsed, total_elapsed])
		return true
	return false
