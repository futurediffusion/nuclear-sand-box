...
func setup(ctx: Dictionary) -> WorldCadenceCoordinator:
	...
	_coordinator.configure_lane(&"bandit_group_scan_slice", BanditTuning.group_scan_interval() / float(maxi(BanditGroupIntel.GROUP_SCAN_SLICE_COUNT, 1)), 0.24)
	...
	_coordinator.configure_lane(&"settlement_base_scan", SettlementIntel.BASE_RESCAN_INTERVAL, SettlementIntel.BASE_SCAN_PHASE_RATIO, 1)
